...
def retrieve_env_vars() -> tuple:
    ...
    try:
        endpoint = os.environ["AZURE_AI_PHI_CHAT_ENDPOINT"]
        key = os.environ["AZURE_AI_PHI_CHAT_KEY"]
    except KeyError:
        logging.error(
            "Missing environment variable 'AZURE_AI_PHI_CHAT_ENDPOINT' or 'AZURE_AI_PHI_CHAT_KEY'"
        )
        logging.error("Set them before running this sample.")
        exit()

    try:
        model_deployment = os.environ["AZURE_AI_PHI_CHAT_DEPLOYMENT_NAME"]
    except KeyError:
        logging.error(
            "Could not read optional environment variable `AZURE_AI_PHI_CHAT_DEPLOYMENT_NAME`."
        )
...
def main() -> None:
                            ...
                            content=[
                                ...
                                ImageContentItem(
                                    image_url=ImageUrl.load(
                                        # image_file=f"data:image/{file_format};base64,{image_base64}",
    ...
    try:
        with open(f"output/image_quality_azure_microsoft_{MODEL_ID}.json", "w") as f:
