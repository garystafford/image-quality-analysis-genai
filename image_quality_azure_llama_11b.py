...
def retrieve_env_vars() -> tuple:
    ...
    try:
        endpoint = os.environ["AZURE_AI_LLAMA11B_CHAT_ENDPOINT"]
        key = os.environ["AZURE_AI_LLAMA11B_CHAT_KEY"]
    except KeyError:
        logging.error(
            "Missing environment variable 'AZURE_AI_LLAMA11B_CHAT_ENDPOINT' or 'AZURE_AI_LLAMA11B_CHAT_KEY'"
        )
        logging.error("Set them before running this sample.")
        exit()

    try:
        model_deployment = os.environ["AAZURE_AI_LLAMA11B_CHAT_DEPLOYMENT_NAME"]
    except KeyError:
        logging.error(
            "Could not read optional environment variable `AZURE_AI_LLAMA11B_CHAT_DEPLOYMENT_NAME`."
        )
